...
export default function SectionEventsScreen({ navigation }) {
	...
	loadEvents = async () => {
		try {
			setLoading(true);
			const response = await eventdatesApi.getEventDates();
			clearData = async () => {
				try {
					await AsyncStorage.removeItem('eventsStored');
				} catch (e) {}

				// console.log('Events db cleared reload.');
		...
		const eventDBStored = await AsyncStorage.getItem('eventsStored');
		...
		setDataEventsFiltered(eventsStored);
	};
